...
import com.sharobi.restaurantapp.model.PosModulesChild;
...
public class LoginDAOImpl implements LoginDAO {
  ...
  private EntityManagerFactory entityManagerFactory = PersistenceListener.getEntityManager();
	...
	public Customer getUserByCredential(Customer customer) throws DAOException {
		...
		if (CommonProerties.testLicense) {
			try {
				...
				transaction.begin();
				TypedQuery<KeyBean> qry = em.createQuery("SELECT k FROM KeyBean k", KeyBean.class);
				...
				if (listKey != null && listKey.size() == 0) {
					...
					cust.setId(0);
					cust.setStatus("N");
					cust.setMessage("You have no license! Please contact customer support for licensing");
					return cust;
				}
		...
		if (CommonProerties.testLicense)
			try {
				// if (isLicense.equalsIgnoreCase("Y"))
				{
					em = entityManagerFactory.createEntityManager();
					EntityTransaction transaction = em.getTransaction();
					transaction.begin();
					...
					qry.setParameter("store_id",
							Integer.toString(loggedCust.getStoreId()));
					...
					if (!(Integer.parseInt(key.getStore_id()) == storeMaster
						...
						loggedCust = new Customer();
						loggedCust.setId(0);
						loggedCust.setMessage("License: Store Id Not Matching");
						loggedCust.setStatus("N");
					...
					if (!(DateUtil.StringDateTojavaDate(key.getTo_date(),"yyyy-MM-dd")).after(DateUtil.StringDateTojavaDate(currentDate,"yyyy-MM-dd"))) {
						loggedCust = new Customer();
						loggedCust.setId(0);
						loggedCust.setMessage("License Expired");
						loggedCust.setStatus("N");
						return loggedCust;
					}
					CustomerDAOImpl customerDAO = new CustomerDAOImpl();
					List<Customer> noOfUsersList = customerDAO
							.getUsersByStore("" + storeMaster.getId());
					int noofUsers = noOfUsersList.size();
					if (!(noofUsers <= Integer.parseInt(key.getNo_of_users()))) {
						loggedCust = new Customer();
						loggedCust.setId(0);
						loggedCust
								.setMessage("Exceeds the Number Of Licensed Users");
						loggedCust.setStatus("N");
						return loggedCust;
					}
				}
				...
				loggedCust.setId(0);
				loggedCust
						.setMessage("You have no license! Please contact customer support for licensing");
			}
		return loggedCust;
	}
