...
class RefundBillPrintArabic implements Printable {
	...
	public int print(Graphics graphics, PageFormat pageFormat, int pageIndex)
		...
		try {
			...
			if(storeId == 136){
			//y=y+10;
			g2d.drawString("_بمبي حاجي على ستار كافیة",25, y);
			}
			double start = 0.0;
			y = y + space ;
			...
			if (store.getInvoiceText() != null) {
				if (store.getInvoiceText().length() > 0
					...
					if(storeId == 136){
					y=y+10;
					g2d.drawString("الفاتورة الضربية",50, y);
		...
		if(store.getRateInBill().equalsIgnoreCase("Y")){			//with rate
			...
			if (store.getServiceChargeText() != null
				...
				//arabic
				cH=cH+10;
				g2d.drawString("تكلفة الخدمة", 00, cH);
			...
			/*if (paidAmtVal.length() > 0) {
				...
				if (paidAmt > 0.0) {
					...
					//arabic
					cH = cH + 10;
					g2dItemTitle.drawString("المبلغ المدفوع", 00, cH);
				}
			}
			g2dItemTitle.setFont(fontItemTitle);
				...
				if (store.getParcelAddress().equalsIgnoreCase("Y")) {
					...
					if (custPhone != null) {
						if (!custPhone.equalsIgnoreCase("")
							...
							//arabic
							cH = cH + 10;
							g2d.drawString("رقم هاتف العميل" , 00, cH);
