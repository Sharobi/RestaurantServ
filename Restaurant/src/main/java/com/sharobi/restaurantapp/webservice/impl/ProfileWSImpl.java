...
import com.sharobi.restaurantapp.model.CustomerInfo;
...
import com.sharobi.restaurantapp.service.CustomerService;
import com.sharobi.restaurantapp.service.exception.ServiceException;
import com.sharobi.restaurantapp.webservice.ProfileWS;
...
public class ProfileWSImpl implements ProfileWS {
  ...
  @Autowired
	...
	@RequestMapping(value = "/getRoomBookingCustomerByPh", method = RequestMethod.GET, consumes = "application/json", produces = "text/plain")
	public String getRoomBookingCustomerByPh(@RequestParam(value = "ph") String ph,
			@RequestParam(value = "storeId") String storeId) {
		RoomBookingCustomer customer = null;
		try {
			customer = customerService.getRoomBookingCustomerByPh(ph, storeId);
		} catch (ServiceException e) {
			e.printStackTrace();
		}
		Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation()
				.create();
		String json = gson.toJson(customer, RoomBookingCustomer.class);
		// System.out.println(json);
